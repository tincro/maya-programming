...
# We'll be making our own node by deriving from MPxNode, a base class for all nodes
class MinMaxNode(om.MPxNode):
    ...
    # For developing your own nodes, that you won't release, you are free to use any number
    # between 0x00000 to 0x7ffff
    # If you plan to release nodes to the public, you need to request a set of ID's from Autodesk
    kNodeID = om.MTypeId(0x01010)

    # We'll also create placeholders for our attrbiutes
    inputA = None
    inputB = None
    mode = None
    output = None
    ...
    # We also need an initialize method that will set up the plugs and attributs on our nodes
    @staticmethod
    def initialize():
        ...
        # Let's start by defining our two input attributes
        # We assing this to an attribute on the class
        ...
        # the nAttrobject remembers the last object created
        ...
        # Finally we need to tel lour node which attributes affect the output of other attributes
        # If we don't do this then our attributes won't update dynamically
        # In this case both our input afect the output
        ...
        def compute(self, plug, data):
            # We handle the copmutation differently based on which node is requested
            ...
            # Finally we set out node to clean, meaning that tis data is fresh
            data.setClean(plug)
...
def initializePlugin(plugin):
    ...
    try:
        pluginFn.registerNode(
            ...
            MinMaxNode.kNodeID, # The unique ID of the node
            MinMaxNode.creator, # the function to create this node
            MinMaxNode.initialize, # The function to intialize the node
        )
    except:
...
def uninitializePlugin(plugin):
    ...
    try:
        ...
        om.MGlobal.displayError('Failed to deregister node %s' % MinMaxNode.kNodeName)
        raise
"""
To load
